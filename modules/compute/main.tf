...
resource "azurerm_network_interface_backend_address_pool_association" "collector" {
  for_each = var.collectors

  network_interface_id    = azurerm_network_interface.collector[each.key].id
  ip_configuration_name   = var.network_interface_ip_configuration_name
  backend_address_pool_id = var.backend_pool_id
}

resource "azurerm_windows_virtual_machine" "collector" {
  for_each = var.collectors
  ...
  network_interface_ids = [
    azurerm_network_interface.collector[each.key].id,
  ]
